...
def show_lidar_with_boxes(pc_velo, objects, calib,
    ...
    for obj in objects:
        ...
        draw_gt_boxes3d([box3d_pts_3d_velo], fig=fig, draw_text=False)
...
def show_lidar_on_image(pc_velo, img, calib, img_width, img_height):
    ...
    cmap = plt.cm.get_cmap('hsv', 480)
    cmap = np.array([cmap(i) for i in range(480)])[:,:3]*255
    ...
    cv2.imshow("lidar_on_image", img)
    return img

def dataset_viz():
    dataset = kitti_object(os.path.join(ROOT_DIR, 'detector/data/kitti'))
